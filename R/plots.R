...
plots$s8_oos_is_corr <- cf_compare %>%
  ...
  labs(title = bquote("Correlation of " * CF * " and " * CF[oos] * " per country"),
...
plots$s8_rx_vs_cf_oos <- reg_data_oos %>%
  ...
  facet_wrap(~ country, ncol = 3, scales = "free") +
  labs(title = bquote("Realized 12m excess return vs " * CF[oos]),
       subtitle = "Out-of-sample predictive scatter per country",
       x = expression(CF[oos]), y = expression(rx[t+12])) +
  theme_thesis
...
plots$s8_r2_oos_local <- r2_oos_tab %>%
  ...
  labs(title = bquote("Campbell-Thompson " * R[oos]^2 * ": local-currency returns"),
...
plots$s8_r2_oos_pooled <- r2_oos_pooled %>%
  ...
  labs(title = bquote("Pooled Campbell-Thompson " * R[oos]^2 * " across G10"),
       subtitle = "SS aggregated over countries; each country uses its own recursive-mean benchmark",
       x = NULL, y = expression(R[oos]^2 ~ "(pooled)")) +
  theme_thesis +
...
plots$s10_r2_oos_compare <- r2_oos_tab %>%
  ...
  labs(title = bquote("Campbell-Thompson " * R[oos]^2 * ": cycle vs forward factors"),
       subtitle = "Recursive factor forecast vs recursive-mean benchmark, 5y min training, 12m horizon",
       x = NULL, y = expression(R[oos]^2)) +
  theme_thesis +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
