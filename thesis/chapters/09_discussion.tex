...
hedged developed-market bond \emph{returns} have a strong factor
structure---three principal components explain $86\%$ of their
variance---yet these common sources of variation are largely
\emph{unpriced}: they account for only about a third of the variation in the
mean--variance-efficient portfolio that prices the asset class. Common
variation and common compensation are different objects. The integration
documented in this thesis concerns the second: a single global factor
describes the \emph{conditional premium}---the time-variation in expected
returns---across the G10. That is fully compatible with much of the common
realised-return variation being unpriced noise; indeed, it is the
...
forecastable component rather than of return co-movement at large.
...
calm integration, and the limit of that rule is sovereign stress. The
timing of the premium also lines up with independent evidence:
\citet{randl2025} estimate that the market price of risk of the hedged
international bond asset class peaks in the financial crisis, the
2010--2012 euro-area crisis, and the 2022 inflation shock, and that it is
related to trend inflation and, after 2008, to the \emph{dispersion} of
inflation across markets---the same crisis windows in which the cycle
factors' explanatory power concentrates here
(\Cref{sec:rob-subsample}), and a further sign that international bond
premia are anchored in inflation dynamics.
...
independent information to add.

A plausible economic explanation lies in
...
market-capitalisation, principal-component) were not explored. An
...
instead---a cross-section chosen for currency diversity rather than debt
size. The two universes are complementary: theirs avoids the euro-bloc
redundancy, while the one used here is what makes the periphery
...
the standpoint of cycle-based predictability, uncompensated noise. This
prescription does not rest on the cycle evidence alone:
\citet{randl2025} reach it from the pricing side, showing that the
mean--variance-efficient hedged bond portfolio carries economically
negligible currency exposure and that the priced risks of international
bonds are fundamentally distinct from those of currencies---the bond
premium and the currency premium are separate objects, best harvested
separately.
...
gives no licence for country selection on local cycle factors.
