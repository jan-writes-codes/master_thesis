...
\subtitle{Supervisor follow-up: is the FXGCF just a re-labelled GCF?}
...
\begin{frame}{The concern: the FXGCF is (almost) the GCF}
  \textbf{Feedback.} In the current thesis the FX-adjusted global cycle factor
  ...
  \begin{itemize}
    \item \citet{dahlquist2013}, our template, report
      ...
      carry \emph{distinct}, currency-driven information.
    \item At $0.99$ the FX adjustment looks cosmetic: the ``novel'' factor is a
      near-affine re-scaling of $\GCF$ and adds little economic content.
  \end{itemize}
  \vfill
  \begin{block}{What this deck does}
    ...
    \begin{enumerate}
      ...
      \item how does its \emph{predictive power} change --- in sample and,
        crucially, out of sample?
    \end{enumerate}
    Then recommend a construction.
  \end{block}
\end{frame}

\begin{frame}{Where the $0.99$ comes from: an asymmetry}
  The local-currency $\GCF$ is built \alert{bottom-up}; the current $\FXGCF$ is
  built \alert{top-down}. They are not symmetric.
  \vfill
  ...
  cycle aggregates that drive $\GCF$ --- so it is collinear with $\GCF$ almost by
  construction.
\end{frame}
...
\begin{frame}{Three ways to build the FXGCF}
  ...
  \]
  \vspace{-0.5ex}
  \begin{block}{Control: M4 = Bottom-up, $1/n$}
    Completes the $2\times2$ (construction $\times$ weighting), so we can tell
    whether any decoupling comes from the \emph{weighting} or the
    \emph{top-down/bottom-up} choice.
  \end{block}
\end{frame}

\begin{frame}{Why the current factor is a $\GCF$ clone (intuition)}
  Write each local factor with its own loadings,
  $\CF_{i}=g_{0i}+g_{1i}\cyc^{(1)}_{i}+g_{2i}\cycbar_{i}$. Because the loadings
  are similar across the G10 (the \citet{cieslak2015} sign pattern holds
  everywhere),
  \begin{align*}
    \GCF_t &\approx \bar g_0+\bar g_1\,\bar c^{(1)}_t+\bar g_2\,\bar{\cycbar}_t,\\
    \FXGCF^{\mathrm{TD}}_t &= h_0+h_1\,\bar c^{(1)}_t+h_2\,\bar{\cycbar}_t.
  \end{align*}
  Both are affine in the \alert{same two global cycle series}
  $(\bar c^{(1)}_t,\bar{\cycbar}_t)$.
  \vfill
  \begin{itemize}
    \item The FX leg only changes the slopes $(h_1,h_2)$ vs.\ $(\bar g_1,\bar g_2)$.
    \item The realized currency return is largely \emph{unpredictable} by bond
      cycles, so the \emph{fitted} component barely rotates $\Rightarrow$
      $\rho\approx0.99$.
    \item \textbf{Bottom-up breaks this}: it aggregates country-specific fitted
      values whose FX-adjusted loadings differ market by market, so the result is
      no longer a simple affine map of the two global aggregates.
  \end{itemize}
\end{frame}
