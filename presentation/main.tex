...
\begin{itemize}
    ...
    \item \textbf{Factors}: country-level CF, GDP-weighted GCF, FX-adjusted FXGCF -- with fully-OOS variants ($CF^{oos}, GCF^{oos}, FXGCF^{oos}$).
...
%
%\vspace{0.2cm}
%\small Country fixed effects; DK lag $=\max(18,\,1.3\sqrt{T})$
%\end{frame}
%
...
\begin{itemize}
    ...
    \item $GCF^{oos}_t = \sum_i w_{i,t}\,CF^{oos}_{i,t}$ 
...
\small Benchmark: mean of $rx$ and $rx^{USD}$.
\end{frame}
...
\begin{enumerate}
    ...
    Adressed in the OOS-factors. Rechecking of values and ensuring there is no look ahead bias.

    \item \textbf{Multiple testing.}
    Robustness checks, adjusting p-values for repeated testing
...
\begin{enumerate}
    ...
    Statistical $R^2$ vs.\ a portfolio metric.
...
\begin{itemize}
    \item OOS results -- which lookback period is adequate?
    \item Is a CER exercise adequate?
\end{itemize}
\end{frame}
...
%\begin{frame}{Residual ACF: $rx\sim CF$}f
%\centering
